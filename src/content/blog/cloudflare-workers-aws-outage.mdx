...
1. **Checked application logs** — Nothing unusual, no crashes or errors
2. **Tested the direct herokuapp.com URL** — It responded! The app was actually running fine
3. **Checked our other environments** — Dev and staging were working normally (they're hosted in Heroku's Europe region)
4. **Correlation** — Europe working, us-east-1 not working... this pointed to the AWS outage we'd been hearing about
...
1. **WebSocket handling**: Our API uses WebSockets for real-time features. While Cloudflare Tunnels technically support WebSockets, we encountered issues with WebSocket upgrade requests not working reliably through our Zero Trust configuration. The initial HTTP handshake worked, but establishing and maintaining WebSocket connections was problematic—likely due to how Zero Trust Gateway handles connection upgrades.
