...
[See it running here](https://lnkd.in/gp38k57p). This is a real vinext app, built with Vite, deployed through Zephyr, running on the edge. Not a synthetic benchmark. Not a staged demo. Just the actual developer experience.
...
## Module Federation Without the Framework Friction

This is where the shift becomes strategically interesting.

Teams running Module Federation, microfrontends, shared component registries, or multi-team frontend platforms know the friction of tightly-coupled framework runtimes. They've lived through breaking bundler changes that require weeks of migration work. They've dealt with runtime rewrites that break remote entry loading. They've hit compatibility freezes where they can't upgrade the framework because it would break the federation layer.

A Vite-first architecture changes that dynamic entirely. Federation thrives in composable environments. It needs predictable bundling behavior, clean remote entry handling, and deployment cycles that don't require coordination across every federated team. vinext + Vite + Zephyr provides exactly that: predictable bundling through Rollup (or Rolldown in Vite 8), cleaner remote entry handling through standard ESM, edge-native SSR compatibility that doesn't require runtime shims, and federation-safe deployment cycles where each microfrontend can deploy independently without breaking the shell.

**How federation works at the edge:** When your app builds, Zephyr generates a `zephyr-manifest.json` that maps each federated dependency to its resolved `remote_entry_url`. At runtime, the Zephyr plugin fetches this manifest and creates import maps that resolve module requests to the correct edge URLs. This happens through a global `__ZEPHYR_MANIFEST_CACHE__` shared across all bundles—so if your shell and your remote both need the same dependency's manifest, it only fetches once.

**Version pinning you can trust:** Each snapshot gets an immutable `version_url` signed with a content hash. You can reference remotes as `app-name@url` and Zephyr resolves the exact version. No more guessing whether you're getting the remote you asked for. Since each version lives on a unique hostname derived from its hash, cross-version contamination is impossible—your v1.2.3 isolate literally cannot access v1.2.4's code.

**Isolation by design:** Beyond version-specific hostnames, Zephyr uses Module Federation's share scopes to control what gets shared between remotes. Each app version runs in its own worker isolate with its own module resolution. If one microfrontend crashes, it doesn't take down the shell or other remotes. For platform teams, this means you can let teams deploy independently without coordinating bundler versions or praying the shared dependencies don't conflict.
...
**Try it:** [Live demo](https://lnkd.in/gp38k57p) | [vinext on GitHub](https://github.com/cloudflare/vinext) | [Zephyr Cloud](https://zephyr-cloud.io)
