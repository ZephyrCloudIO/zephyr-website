...
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Blog post                 | Add MDX to `src/content/blog/`, images to `src/images/blog/<slug>/`, update authors/images/loaders only if needed, then run `pnpm run generate:rspress-content`. The two newest posts are auto-featured; there is no `featured` flag.                        |
| Changelog entry           | Add MDX to `src/content/changelog/`, then run `pnpm run generate:rspress-content` (entries are auto-discovered by scanning the source dir; no manual registration).                                                                                          |
