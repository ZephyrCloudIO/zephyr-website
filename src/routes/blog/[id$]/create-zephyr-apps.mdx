...
How developers would publish an app to App Stores doesn't change. How cross-platform UI can be crafted stay the same ([\<u>you can use \</u>](https://x.com/_jbroma/status/1881452224305574233?s=46)**[\<u>NativeWind\</u>](https://x.com/_jbroma/status/1881452224305574233?s=46)\*\***\<u> \</u>\***\*!!!**) The changes happens in build configuration by replacing `metro.config.js` to `rspack.config.ts` - you can read more about the [how-to in our documentation](https://docs.zephyr-cloud.io/recipes/repack-mf).

(consider moving the content between line break to documentation)
